...
const blank=()=>({version:"5.4.5",space:"work",nodes:[],links:[],inbox:[],updated:Date.now()});
...
async function fullResetApp(){
 ...
 location.replace(location.pathname+'?v=5.4.5&reset='+Date.now());
}
...
if("serviceWorker" in navigator){
 window.addEventListener("load",()=>{
  navigator.serviceWorker.register("sw.js?v=5.4.5",{updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{});
 });
