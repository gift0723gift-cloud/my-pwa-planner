(() => {
  ...
  const PROJECT_ICON_PATHS = {
    ...
    link:'<path d="M10 14 8 16a4 4 0 1 1-5.7-5.6l3-3A4 4 0 0 1 11 8"/><path d="M14 10l2-2a4 4 0 1 1 5.7 5.6l-3 3A4 4 0 0 1 13 16"/><path d="m9 15 6-6"/>',
  ...
  function hydrateInlineIcons(root=document){root.querySelectorAll('[data-inline-icon]').forEach(el=>{el.innerHTML=projectIcon(el.dataset.inlineIcon,'line-icon');});}
  ...
  function renderTree(){
    ...
    visible.forEach(n=>{
      ...
      if(n.minimized){
        if(n.type==='project'){
          const pd=defaultProjectData(n.projectData), note=pd.preliminaryInfo?shortText(pd.preliminaryInfo,84):'Заполните карточку проекта';
          el.classList.add('project-tree-node',`project-status-${pd.status}`);
          el.innerHTML=`
            <div class="project-node-header-row">
              <div class="project-node-cover"><img data-project-card-image="${n.id}" alt=""><span>${projectIcon('project','project-cover-icon')}</span></div>
              <div class="project-node-head-copy">
                <span class="node-type">${projectIcon('project','node-type-svg')} Проект</span>
                <div class="node-title">${escapeHtml(n.title)}</div>
                <div class="project-node-status compact"><span>${escapeHtml(PROJECT_PRIORITY_LABELS[pd.priority])}</span><span>${escapeHtml(PROJECT_STATUS_LABELS[pd.status])}</span></div>
              </div>
              <div class="node-top-badges project-badges">${n.locked?'<span title="Зафиксировано">⌖</span>':''}<button class="node-more" aria-label="Удерживайте блок">•••</button></div>
            </div>
            <div class="project-node-note compact">${escapeHtml(note)}</div>
            <div class="project-node-facts compact">${pd.address?`<span>${projectIcon('goal','tiny-svg')} ${escapeHtml(shortText(pd.address,26))}</span>`:''}${pd.positions?`<span>${projectIcon('stage','tiny-svg')} ${escapeHtml(pd.positions)} поз.</span>`:''}${pd.budget!==''?`<span>${projectIcon('finance','tiny-svg')} ${formatMoney(pd.budget)}</span>`:''}${pd.signingDate?`<span>${projectIcon('note','tiny-svg')} ${formatDate(pd.signingDate)}</span>`:''}</div>
            <div class="node-bottom"><span>${prog}%</span><span class="data-counts"><span>${projectIcon('people','tiny-svg')} ${contacts}</span><span>${projectIcon('image','tiny-svg')} ${n.images.length}</span><span>${projectIcon('pdf','tiny-svg')} ${n.pdfs.length}</span><span>${projectIcon('file','tiny-svg')} ${n.files.length}</span></span></div>
            <div class="progress-track"><div class="progress-fill" style="width:${prog}%"></div></div>
            ${kids.length?`<div class="node-hint">${kids.length} связанных элементов</div>`:'<div class="node-hint">Нажмите для просмотра · удерживайте для панели</div>'}`;
        }else{
  ...
  function showNodeControls(id,nodeEl){
    ...
    const childLabel=els.controlPanel.querySelector('[data-node-action="child"] small');
    if(childLabel) childLabel.textContent=['project','goal','person','idea'].includes(n.type)?'Этап':'Задача';
  ...
  function toolbarLinkAction(){
    if(linkSourceId){cancelLink();showToast('Режим связи отключён');return;}
    if(!selectedId){showToast('Сначала выберите элемент');return;}
    const n=getNode(selectedId); if(!n) return;
    if(n.parentId){n.parentId=null;save({skipDrafts:true});renderAll();showToast('Связь снята. Элемент стал корневым');return;}
    startLinkMode(selectedId);
  }
  function updateLinkModeUI(){els.linkBar.classList.toggle('hidden',!linkSourceId);if(linkSourceId){const n=getNode(linkSourceId);$('linkModeText').textContent=`Связь: «${n?.title||''}» → выберите новый родительский блок`;} const btn=$('quickLinkBtn'); if(btn){btn.classList.toggle('active',!!linkSourceId); const t=btn.querySelector('small'); if(t) t.textContent=linkSourceId?'Отмена':'Связать';}}
  ...
  function addNode(parentId=null,rootType=null){
    ...
    const type = !p ? (rootType||'goal') : (['project','goal','person','idea'].includes(p.type) ? 'stage' : 'task');
    const titles={project:'Новый проект',goal:'Новая цель',person:'Новый человек',stage:'Новый этап',idea:'Новая идея',task:'Новая задача'};
  ...
  $('quickProjectBtn').addEventListener('click',()=>openProjectSetup('add')); $('quickGoalBtn').addEventListener('click',()=>addNode(null,'goal')); $('quickPersonBtn').addEventListener('click',()=>addNode(null,'person')); $('quickIdeaBtn').addEventListener('click',()=>addNode(null,'idea')); $('quickLinkBtn').addEventListener('click',toolbarLinkAction); $('emptyAddBtn').addEventListener('click',()=>state.activeSection==='projects'&&!state.activeProjectId?openProjectSetup('onboarding'):addNode(null,'goal'));
  ...
  { let timer=null,longPress=false; const b=$('projectSwitcherBtn'); b.addEventListener('pointerdown',()=>{longPress=false; timer=setTimeout(()=>{longPress=true; openProjects(); navigator.vibrate?.(15);},520);}); ['pointerup','pointercancel','pointerleave'].forEach(ev=>b.addEventListener(ev,()=>clearTimeout(timer))); b.addEventListener('click',()=>{ if(longPress) return; fitTree(); showToast('Текущий проект в фокусе'); }); }$('quickAddProjectBtn').addEventListener('click',()=>openProjectSetup('add'));$('addProjectFromListBtn').addEventListener('click',()=>openProjectSetup('add'));$('deleteProjectBtn').addEventListener('click',deleteActiveProject);$('createProjectBtn').addEventListener('click',createProject);$('cancelProjectSetupBtn').addEventListener('click',()=>closeAllOverlays());
