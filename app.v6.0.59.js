...
function handleGlobalCardAction(event) {
  ...
  if (action === "open") return openDetail(node);
  if (action === "connect") { state.linkCreateSourceId = node.id; panel.classList.add("hidden"); syncCardSelectionDOM(); renderLinks(); toast("Выберите карточку для связи"); return; }
