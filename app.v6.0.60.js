...
const state = {
  ...
  canvasGesture: null,
  lastCanvasTap: 0,
  lastCardTap: { id: null, time: 0, x: 0, y: 0 },
...
function bindWorkspaceOnce() {
  ...
  $("#linkLayer").addEventListener("pointerup", event => { if (event.target.closest?.(".link-hit,.link-path,.link-end-handle")) { event.stopPropagation(); state.suppressCanvasTapUntil = performance.now() + 800; } }, true);
  $("#linkLayer").addEventListener("pointercancel", event => { if (event.target.closest?.(".link-hit,.link-path,.link-end-handle")) { event.stopPropagation(); state.suppressCanvasTapUntil = performance.now() + 800; } }, true);
  ...
  $("#globalCardActions")?.addEventListener("click", handleGlobalCardAction);
...
function syncCardSelectionDOM() {
  ...
  syncGlobalCardActions();
}

function syncGlobalCardActions() {
  const panel = $("#globalCardActions");
  if (!panel) return;
  const node = nodeById(state.selectedId);
  const card = node ? $(`.node-card[data-id="${CSS.escape(node.id)}"]`, $("#cardLayer")) : null;
  if (!node || !card || state.linkCreateSourceId || card.classList.contains("dragging")) {
    panel.classList.add("hidden");
    panel.dataset.nodeId = "";
    return;
  }
  const left = Number.parseFloat(card.style.left) + card.offsetWidth / 2;
  const top = Number.parseFloat(card.style.top) + card.offsetHeight + 8;
  panel.style.left = `${left}px`;
  panel.style.top = `${top}px`;
  panel.dataset.nodeId = node.id;
  const openButton = panel.querySelector('[data-global-card-action="open"]');
  const connectButton = panel.querySelector('[data-global-card-action="connect"]');
  const lockButton = panel.querySelector('[data-global-card-action="toggleLock"]');
  if (openButton) openButton.innerHTML = icon("open");
  if (connectButton) connectButton.innerHTML = icon("link");
  if (lockButton) {
    lockButton.innerHTML = icon(node.locked ? "lock" : "unlock");
    lockButton.setAttribute("aria-label", node.locked ? "Снять фиксацию" : "Зафиксировать карточку");
  }
  panel.classList.remove("hidden");
}

function handleGlobalCardAction(event) {
  const button = event.target.closest("[data-global-card-action]");
  if (!button) return;
  event.preventDefault(); event.stopPropagation();
  const panel = $("#globalCardActions");
  const node = nodeById(panel?.dataset.nodeId || state.selectedId);
  if (!node) return;
  const action = button.dataset.globalCardAction;
  if (action === "open") {
    const now = performance.now();
    if (state.eyeTap.id === node.id && state.eyeTap.timer && now - state.eyeTap.time < 380) {
    ...
    state.eyeTap = { id: node.id, time: now, timer: setTimeout(() => {
      state.eyeTap = { id: null, timer: null, time: 0 };
  ...
  if (action === "connect") { startLinkCreation(node); return; }
  if (action === "toggleLock") { node.locked = !node.locked; saveData(); renderCards(); renderLinks(); toast(node.locked ? "Карточка зафиксирована" : "Фиксация снята"); }
}
...
function attachCardGestures(element, node) {
  ...
  element.addEventListener("pointerdown", event => {
    ...
    const priorTap = state.lastCardTap;
    const closeEnough = priorTap.id === node.id && tapNow - priorTap.time < 500 && Math.hypot(event.clientX - (priorTap.x || event.clientX), event.clientY - (priorTap.y || event.clientY)) < 28;
    if (closeEnough && pointers.size === 0) {
      state.lastCardTap = { id: null, time: 0, x: 0, y: 0 };
      event.currentTarget.classList.add("tap-opening");
      setTimeout(() => openDetail(node), 0);
      return;
    }
    ...
    if (pointers.size === 2) {
      cancelLong();
      $("#globalCardActions")?.classList.add("hidden");
  ...
  const finish = event => {
    ...
    if (gesture?.type === "drag") {
      ...
      saveData(); renderLinks(); syncGlobalCardActions();
      ...
      const now = performance.now();
      state.lastCardTap = { id: node.id, time: now, x: event.clientX, y: event.clientY };
...
function handleDetailClick(event) {
  ...
  if (personTask) {
    ...
    if (process) { closeDetail(); state.selectedId = process.id; render(); focusNode(process); openDetail(process); requestAnimationFrame(() => { const row = document.querySelector(`[data-stage-task-id="${CSS.escape(personTask.dataset.personTaskId)}"]`); row?.classList.add("focus-flash"); setTimeout(() => row?.classList.remove("focus-flash"), 950); }); }
    return;
  }
  const relation = event.target.closest("[data-person-relation-id]");
  if (relation) { const target = nodeById(relation.dataset.personRelationId); if (target) { closeDetail(); state.selectedId = target.id; render(); focusNode(target); openDetail(target); } return; }
